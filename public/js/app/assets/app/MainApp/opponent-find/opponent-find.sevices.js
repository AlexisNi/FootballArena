...
export var OpponentFindService = (function () {
    ...
    OpponentFindService.prototype.createArena = function (arenaPlayer) {
        ...
        return this.http.post(myGlobals.host + 'arena' + token, body, { headers: headers })
            ...
            var arenaUsers = new ArenaUsers(response.json().obj._id, response.json().obj.user._id, response.json().obj.invite._id, response.json().obj.status_accept, response.json().obj.invite.lastName, response.json().obj.user_played, response.json().obj.invite_played, response.json().obj.questions);
