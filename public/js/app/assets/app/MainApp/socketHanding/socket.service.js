...
export var SocketService = (function () {
    ...
    SocketService.prototype.test = function () { this.socket.emit('test'); };
    ;
        ...
        var observable = new Observable(function (observer) {
            _this.socket.on('loadArenas', function (data) {
                ...
                for (var _i = 0, arenas_1 = arenas; _i < arenas_1.length; _i++) {
                    ...
                    transformedArenas.push(new ArenaUsers(arena._id, arena.user, arena.invite._id, arena.status_accept, arena.user.lastName || arena.invite.lastName, arena.user_played, arena.invite_played, arena.questions));
                }
                var UserArenas = data.objUser;
                for (var _a = 0, UserArenas_1 = UserArenas; _a < UserArenas_1.length; _a++) {
                    var userArena = UserArenas_1[_a];
                    transformedArenas.push(new ArenaUsers(userArena._id, userArena.user._id, userArena.invite, userArena.status_accept, userArena.user.lastName, userArena.user_played, userArena.invite_played, userArena.questions));
                }
